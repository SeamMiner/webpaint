export class Paint {
  ...
  private _memCtx: CanvasRenderingContext2D | null = this._memCanvas.getContext('2d');
  ...
  public draw(event: PointerEvent) {
    ...
    this._ctx!.quadraticCurveTo(this._points[this._points.length - 2].x, this._points[this._points.length - 2].y, this._points[this._points.length - 1].x, this._points[this._points.length - 1].y);
    this._ctx!.stroke()
  }
  ...
  private restoreState(history = this._undoHistory, saveTo = this._redoHistory) {
    if (history.length) {
