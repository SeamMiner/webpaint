<template>
  <div class="pickedColorTool">
    <input type="color" class="color" id="colorPicker" v-model="colorPickerColor" :class="{'active': (activeColor == hex2rgb(colorPickerColor)) && !colors.includes(hex2rgb(colorPickerColor))}">
    <template v-for="color in colors" :key="color">
      <div class="color" :style="`background-color: rgb(${color});`" @click="setActiveColor(color)" :class="{'active': activeColor == color}"></div>
    </template>
  </div>
</template>
  ...
  setup() {
    ...
    const hex2rgb = (hex: string) => {
      if (/^#([0-9a-f]{3}|[0-9a-f]{6})$/i.test(hex)) {
        ...
        .map(function(l: string) { return parseInt(hex.length%2 ? l+l : l, 16) })
        .join(' ')
      } else {
        return '0 0 0'
      }
    ...
    watchEffect(() => setActiveColor(
      hex2rgb(colorPickerColor.value)
    ))
...
.pickedColorTool {
  ...
  .color.active::before, input[type="color"]:focus::before {
    content: "";
    position: absolute;
    height: 1rem;
    width: 1rem;
